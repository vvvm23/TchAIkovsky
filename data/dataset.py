...
def get_dataset(min_sequence_length=128, max_sequence_length=1024, subset: float = 1.0):
    ...
    ds = DatasetTok(
        files, min_seq_len=min_sequence_length, max_seq_len=max_sequence_length, one_token_stream = False,
    )
    return ds
...
def generate_splits(dataset, splits: Tuple[float, float]):
    ...
    return torch.utils.data.Subset(dataset, range(split_size)), torch.utils.data.Subset(dataset, range(split_size, length))
